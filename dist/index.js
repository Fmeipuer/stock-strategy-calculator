...
function printExpectedPrice(args) {
...
function parseExpectedPriceArgs(argv) {
    if (argv.length !== 2) {
        throw new Error("参数数量不正确。用法: node dist/index.js expected-price <当前股价> <预期股价>");
    }
...
function parseBuyArgs(argv) {
    if (argv.length !== 2) {
        throw new Error("参数数量不正确。用法: node dist/index.js buy <买入金额> <当前股价>");
    }
...
function parseDayTradeArgs(argv) {
    if (argv.length !== 8) {
        throw new Error("参数数量不正确。用法: node dist/index.js day-trade <原本持仓数量> <持仓成本> <做T买入成本> <买入数量> <买入交易手续费> <做T卖出成本> <卖出数量> <卖出交易手续费>");
    }
...
function parseLimitUpArgs(argv) {
    if (argv.length < 3 || argv.length > 4) {
        throw new Error("参数数量不正确。用法: node dist/index.js [limit-up] <初始本金> <涨停百分比> <目标金额> [最大次数]");
    }
