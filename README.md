...
浏览器页面包含多个常用计算模块；命令行工具提供涨停复利和预期股价涨跌计算。
...
用于根据建仓和补仓的成交价格、买入股数、手续费计算补仓后的平均成本价。
...
```text
最新成本价 = (建仓成本价格 × 建仓买入股数 + 建仓手续费 + 补仓成本价格 × 补仓买入股数 + 补仓手续费) / 总持股数
总持股数 = 建仓买入股数 + 补仓买入股数
```
...
- `initialPrincipal` 必须是大于 0 的数字
- `limitUpPercent` 必须是大于 0 的数字
- `targetAmount` 必须是大于 0 的数字
- `maxSteps` 如果传入，必须是大于 0 的整数
...
- `currentPrice` 必须是大于 0 的数字
- `expectedPrice` 必须是大于 0 的数字
